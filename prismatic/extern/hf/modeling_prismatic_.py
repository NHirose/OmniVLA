...
class PrismaticForConditionalGeneration_MMN(PrismaticPreTrainedModel):
    def __init__(self, config: PrismaticConfig) -> None:
        super().__init__(config)
    ...
    def _build_multimodal_attention_MMN(self, input_embeddings, projected_patch_embeddings, goal_encoding_img_4096, goal_encoding_pose_4096, map_encoding_4096, obs_hist_4096, attention_mask, modality_id):
        """Build multimodal embeddings and attention mask"""
        # Update attention mask
        #projected_patch_attention_mask = None
        ...
        for ib in range(projected_patch_embeddings.shape[0]):
            ...
            projected_patch_attention_mask_b = torch.cat((lan_attention_mask, img_attention_mask, pose_attention_mask, sate_attention_mask, obs_attention_mask), axis=1)
        ...
        # Build multimodal embeddings & attention mask; insert embeddings after <BOS> token (1:)
        multimodal_embeddings = torch.cat(
            [input_embeddings[:, :1, :], projected_patch_embeddings, goal_encoding_img_4096, goal_encoding_pose_4096, map_encoding_4096, obs_hist_4096, input_embeddings[:, 1:, :]], dim=1
        )
        #print("projected_patch_embeddings", projected_patch_embeddings.size())
    ...
    def _build_multimodal_labels(self, labels, projected_patch_embeddings):
        ...
        if labels is not None:
            projected_patch_labels = torch.full(
                (projected_patch_embeddings.shape[0], projected_patch_embeddings.shape[1] + 3 + self.context_size + 1),
                fill_value=IGNORE_INDEX,
                dtype=labels.dtype,
                device=labels.device,
            )
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            #)
            multimodal_embeddings, multimodal_attention_mask = self._build_multimodal_attention_MMN(
                input_embeddings, projected_patch_embeddings, goal_encoding_img_4096, goal_encoding_pose_4096, map_encoding_4096, obs_encoding_4096, attention_mask, modality_id
            )            
            #print("multimodal_embeddings", multimodal_embeddings.size())
            #print(multimodal_attention_mask)
            #print(multimodal_attention_mask.size())
...
class PrismaticForConditionalGeneration_MMNv1(PrismaticPreTrainedModel):
    ...
    def _build_multimodal_attention_MMN(self, input_embeddings, projected_patch_embeddings, attention_mask, modality_id):
        """Build multimodal embeddings and attention mask"""
        # Update attention mask
        #projected_patch_attention_mask = None
        projected_patch_attention_mask = []
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            #)
            multimodal_embeddings, multimodal_attention_mask = self._build_multimodal_attention_MMN(
                input_embeddings, projected_patch_embeddings, attention_mask, modality_id
            )            
            #print("multimodal_embeddings", multimodal_embeddings.size())
            #print(multimodal_attention_mask)
            #print(multimodal_attention_mask.size())
...
class OpenVLAForActionPrediction_MMN(PrismaticForConditionalGeneration_MMN):
    config_class: PretrainedConfig = OpenVLAConfig
